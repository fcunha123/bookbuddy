...
/* ═══════════════════════════ SVG */
const Blob = ({ color, size = 120, style = {} }: { color: string; size?: number; style?: React.CSSProperties }) => (
...
/* ═══════════════════════════ REUSABLE UI */
const Inp = ({ id, label, type = "text", value, onChange, placeholder, required = false, error = "", hint = "" }: {
  ...
  <div style={{ marginBottom: 14 }}>
    ...
    <input id={id} type={type} value={value} onChange={e => onChange(e.target.value)}
      ...
      style={{
        width: "100%", border: `3px solid ${error ? C.coral : C.navy}`, borderRadius: 14,
...
// FIX: outline border was `outline?C.navy:C.navy` — both identical. Now correctly transparent bg.
const Btn = ({ children, onClick, bg = C.coral, color = C.white, disabled = false, outline = false, small = false, style = {} }: {
  children: React.ReactNode; onClick?: () => void; bg?: string; color?: string;
  disabled?: boolean; outline?: boolean; small?: boolean; style?: React.CSSProperties;
}) => (
  <button onClick={onClick} disabled={disabled} style={{
    width: "100%", background: outline ? "transparent" : bg,
    color: outline ? C.navy : color,
    border: `3px solid ${outline ? C.navy : C.navy}`,
    borderRadius: small ? 14 : 18,
    padding: small ? "8px 14px" : "14px",
    fontFamily: "'Fredoka One',cursive",
    fontSize: small ? 14 : 18,
    cursor: disabled ? "not-allowed" : "pointer",
    boxShadow: outline || disabled ? "none" : shB(),
...
const Empty = ({ icon, title, sub, action, onAction }: {
  ...
  <div style={{ textAlign: "center", padding: "40px 24px" }}>
    ...
    {action && <button onClick={onAction} style={{
      background: C.coral, color: C.white, border: `3px solid ${C.navy}`,
      borderRadius: 18, padding: "12px 28px", fontFamily: "'Fredoka One',cursive",
      fontSize: 16, cursor: "pointer", boxShadow: shB(),
    }}>{action}</button>}
  </div>
);
...
const Toast = ({ msg, onDone }: { msg: string; onDone: () => void }) => {
  ...
  return (
    <div role="status" aria-live="polite" style={{
      ...
      zIndex: 999, background: C.yellow, border: `3px solid ${C.navy}`,
      borderRadius: 16, padding: "12px 20px", boxShadow: shB(),
      fontFamily: "'Fredoka One',cursive", fontSize: 15, color: C.navy, whiteSpace: "nowrap",
    }}>
      {msg}
    </div>
  );
...
const ErrBox = ({ msg, onRetry }: { msg: string; onRetry?: () => void }) => (
  <div role="alert" style={{
    background: "#FFE5E5", borderRadius: 18, padding: "16px 18px",
    border: `3px solid ${C.coral}`, margin: "16px 0",
  }}>
    <p style={{ fontFamily: "'Fredoka One',cursive", fontSize: 16, color: C.navy, marginBottom: 4 }}>Erro ⚠️</p>
    <p style={{ fontFamily: "'Boogaloo',cursive", fontSize: 13, color: C.navy, opacity: .8 }}>{msg}</p>
    {onRetry && <button onClick={onRetry} style={{
      marginTop: 10, background: C.coral, color: C.white, border: `2px solid ${C.navy}`,
      borderRadius: 12, padding: "8px 14px", fontFamily: "'Fredoka One',cursive",
...
const Capa = ({ book, size = 90 }: { book: Book; size?: number }) =>
  book.capa_url
    ? <img src={book.capa_url} alt={`Capa: ${book.titulo}`} style={{
        width: size * .75, height: size, objectFit: "cover", borderRadius: 10,
...
// FIX: replaced deprecated document.execCommand with modern Clipboard API + graceful fallback
...
function Welcome({ onLogin, onRegister }: { onLogin: () => void; onRegister: () => void }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100dvh" }}>
      <div style={{
        background: C.coral, flex: 1, display: "flex", flexDirection: "column",
        ...
        <div style={{ position: "relative", zIndex: 1, textAlign: "center" }}>
          <div style={{
            width: 80, height: 80, borderRadius: 24, background: C.white,
            border: `4px solid ${C.navy}`, display: "flex", alignItems: "center",
            justifyContent: "center", fontSize: 40, boxShadow: shB(), margin: "0 auto 20px",
          }}>📚</div>
          <h1 style={{ fontFamily: "'Fredoka One',cursive", fontSize: 42, color: C.white, lineHeight: 1, marginBottom: 8, textShadow: `3px 3px 0 ${C.navy}` }}>BookBuddy</h1>
          <p style={{ fontFamily: "'Boogaloo',cursive", fontSize: 18, color: "rgba(255,255,255,.95)", marginBottom: 28, lineHeight: 1.5 }}>
            Troca livros com os teus amigos de forma segura e divertida! 📖
          </p>
          <Mascot size={160} />
        </div>
        <Wave color={C.cream} />
      ...
      <div style={{ background: C.cream, padding: "28px 24px 40px" }}>
        <div style={{ display: "flex", gap: 8, justifyContent: "center", marginBottom: 24, flexWrap: "wrap" }}>
          {["📚 Empresta livros", "👫 Amigos leitores", "🔒 100% seguro", "🌟 Gratuito"].map(f => (
            <span key={f} style={{
              background: C.white, border: `2.5px solid ${C.navy}`, borderRadius: 20,
              padding: "5px 12px", fontFamily: "'Boogaloo',cursive", fontSize: 12, color: C.navy, boxShadow: sh(C.navy, 1, 2),
            }}>{f}</span>
          ))}
        </div>
        <Btn onClick={onRegister} style={{ marginBottom: 12 }}>🚀 Criar conta grátis</Btn>
        <Btn onClick={onLogin} outline>Já tenho conta</Btn>
      </div>
    </div>
  );
}
...
function Register({ onSuccess, onLogin }: { onSuccess: (p: Profile) => void; onLogin: () => void }) {
  ...
  const registar = async () => {
    ...
    try {
      ...
      setErr((e as Error).message || "Erro ao criar conta. Tenta novamente.");
    }
    setLoading(false);
  };
      ...
      <div style={{ background: C.teal, padding: "20px 20px 0", position: "relative", overflow: "hidden", flexShrink: 0 }}>
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          <button onClick={() => step === 2 ? setStep(1) : onLogin()}
            style={{ background: C.navy, color: C.white, border: "none", borderRadius: 12, padding: "8px 16px", fontFamily: "'Fredoka One',cursive", fontSize: 13, cursor: "pointer", marginBottom: 12, minHeight: 44 }}>← Voltar</button>
          <h1 style={{ fontFamily: "'Fredoka One',cursive", fontSize: 28, color: C.navy }}>{step === 1 ? "Criar conta 🎉" : "Escolhe o teu avatar!"}</h1>
          <p style={{ fontFamily: "'Boogaloo',cursive", fontSize: 14, color: C.navy, opacity: .8, marginBottom: 14 }}>{step === 1 ? "É rápido e gratuito" : "Quem és tu no mundo da leitura?"}</p>
          <div style={{ display: "flex", gap: 6, marginBottom: 4 }}>
            {[1, 2].map(s => <div key={s} style={{ height: 5, flex: 1, borderRadius: 3, background: s <= step ? C.navy : "rgba(0,0,0,.2)" }} />)}
          </div>
        </div>
        <Wave color={C.cream} />
      </div>
      <div style={{ background: C.cream, padding: "16px 20px 40px", flex: 1 }}>
        ...
        {step === 1 ? (
          <>
            ...
            <div style={{ background: C.green + "33", borderRadius: 14, padding: "12px 16px", border: `2.5px solid ${C.green}`, marginBottom: 20, display: "flex", gap: 10 }}>
              <span aria-hidden="true" style={{ fontSize: 20 }}>🔒</span>
              <p style={{ fontFamily: "'Boogaloo',cursive", fontSize: 13, color: C.navy, lineHeight: 1.6 }}>A tua biblioteca é privada — só os teus amigos podem ver os teus livros.</p>
            </div>
          ...
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 24 }}>
              {EMOJIS.map(e => (
                <button key={e} onClick={() => setEmoji(e)} aria-pressed={emoji === e} style={{
                  aspectRatio: "1", fontSize: 36, borderRadius: 18,
            ...
            <div style={{ background: C.yellow, borderRadius: 18, padding: "14px", border: `3px solid ${C.navy}`, boxShadow: sh(C.navy), display: "flex", alignItems: "center", gap: 14, marginBottom: 24 }}>
              <div style={{ width: 52, height: 52, borderRadius: 16, background: C.white, border: `3px solid ${C.navy}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28 }}>{emoji}</div>
              <div>
        ...
        <p style={{ fontFamily: "'Boogaloo',cursive", fontSize: 13, color: C.navy, opacity: .7, textAlign: "center", marginTop: 16 }}>
          Já tens conta?{" "}
          <button onClick={onLogin} style={{ background: "none", border: "none", color: C.coral, fontFamily: "'Fredoka One',cursive", fontSize: 13, cursor: "pointer", textDecoration: "underline" }}>Entrar</button>
        </p>
      </div>
    </div>
  );
...
function Login({ onSuccess, onRegister }: { onSuccess: (p: Profile) => void; onRegister: () => void }) {
  ...
  const entrar = async () => {
    ...
    } catch (e: unknown) { setErr((e as Error).message || "E-mail ou senha incorretos."); }
    setLoading(false);
  };

  const enviarReset = async () => {
    if (!email.includes("@")) { setErr("Introduz o teu e-mail primeiro."); return; }
    setErr(""); setResetLoading(true);
    try {
      await resetPassword(email);
      setResetDone(true);
    } catch (e: unknown) { setErr((e as Error).message || "Erro ao enviar e-mail."); }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100dvh" }}>
      <div style={{ background: C.lavender, padding: "20px 20px 0", position: "relative", overflow: "hidden", flexShrink: 0 }}>
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          <h1 style={{ fontFamily: "'Fredoka One',cursive", fontSize: 30, color: C.navy }}>{resetMode ? "Recuperar senha 🔑" : "Bem-vindo! 👋"}</h1>
          <p style={{ fontFamily: "'Boogaloo',cursive", fontSize: 15, color: C.navy, opacity: .8, marginBottom: 14 }}>
            {resetMode ? "Envia-nos o teu e-mail e enviamos um link" : "Entra para ver a tua biblioteca"}
          </p>
      ...
      <div style={{ background: C.cream, padding: "16px 20px 40px", flex: 1 }}>
        {err && <div role="alert" style={{ background: "#FFE5E5", border: `2.5px solid ${C.coral}`, borderRadius: 14, padding: "10px 14px", marginBottom: 14, fontFamily: "'Boogaloo',cursive", fontSize: 13, color: C.navy }}>⚠️ {err}</div>}
        {resetDone ? (
          <div style={{ background: C.green, borderRadius: 18, padding: "20px", border: `3px solid ${C.navy}`, boxShadow: sh(C.navy), textAlign: "center", marginBottom: 20 }}>
            <p style={{ fontFamily: "'Fredoka One',cursive", fontSize: 20, color: C.navy, marginBottom: 8 }}>📧 E-mail enviado!</p>
            <p style={{ fontFamily: "'Boogaloo',cursive", fontSize: 14, color: C.navy, lineHeight: 1.6 }}>Verifica a tua caixa de entrada e clica no link para redefinir a senha.</p>
          </div>
        ) : resetMode ? (
          <>
            <Inp id="l-email-r" label="O teu e-mail" type="email" value={email} onChange={setEmail} placeholder="francisco@email.com" />
            <Btn onClick={enviarReset} disabled={resetLoading} style={{ marginBottom: 12 }}>{resetLoading ? "A enviar..." : "📧 Enviar link de recuperação"}</Btn>
            <Btn onClick={() => { setResetMode(false); setErr(""); }} outline>← Voltar ao login</Btn>
          </>
        ) : (
          <>
            ...
            <div style={{ textAlign: "right", marginBottom: 20 }}>
              <button onClick={() => { setResetMode(true); setErr(""); }} style={{ background: "none", border: "none", color: C.teal, fontFamily: "'Fredoka One',cursive", fontSize: 13, cursor: "pointer" }}>Esqueci a senha</button>
            </div>
            <Btn onClick={entrar} disabled={loading} style={{ marginBottom: 12 }}>{loading ? "A entrar..." : "Entrar →"}</Btn>
            <Btn onClick={onRegister} outline>Criar conta grátis 🚀</Btn>
          </>
        )}
      </div>
    </div>
...
/* ═══════════════════════════ BOOK DETAIL */
function BookDetail({ book, user, lenderId, onBack, onRequestDone }: {
  book: Book; user: Profile; lenderId: string; onBack: () => void; onRequestDone?: () => void;
}) {
  const [loading, setLoading] = useState(false);
  ...
  const pedir = async () => {
    ...
    try {
      ...
      setDone(true);
      onRequestDone?.();
    } catch (e: unknown) { setErr((e as Error).message || "Erro ao enviar pedido."); }
    setLoading(false);
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100dvh" }}>
      <div style={{ background: C.lavender, padding: "18px 20px 0", position: "relative", overflow: "hidden" }}>
        ...
        <button onClick={onBack} style={{ background: C.navy, color: C.white, border: "none", borderRadius: 16, padding: "10px 20px", fontFamily: "'Fredoka One',cursive", fontSize: 15, cursor: "pointer", boxShadow: sh(C.navy, 2, 3), position: "relative", zIndex: 2, minHeight: 44 }}>← Voltar</button>
        <div style={{ display: "flex", gap: 14, marginTop: 12, position: "relative", zIndex: 1, paddingBottom: 8 }}>
          <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>
            {book.capa_url
              ? <img src={book.capa_url} alt={`Capa: ${book.titulo}`} style={{ width: 120, height: 168, objectFit: "cover", borderRadius: 14, border: `4px solid ${C.navy}`, boxShadow: `6px 8px 0 ${C.navy}`, transform: "rotate(-2deg)" }} />
              : <div style={{ width: 110, height: 155, borderRadius: 14, background: C.white, border: `4px solid ${C.navy}`, boxShadow: `6px 8px 0 ${C.navy}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 56 }}>📚</div>
            }
          </div>
          <div style={{ flex: 1.2, paddingBottom: 20 }}>
            <span style={{ display: "inline-block", background: C.coral, color: C.white, borderRadius: 12, padding: "4px 14px", fontFamily: "'Fredoka One',cursive", fontSize: 12, border: `2px solid ${C.navy}`, boxShadow: sh(C.navy, 2, 2), marginBottom: 8 }}>{book.genero}</span>
            <h1 style={{ fontFamily: "'Fredoka One',cursive", fontSize: 22, color: C.navy, lineHeight: 1.15, marginBottom: 6 }}>{book.titulo}</h1>
      ...
      <div style={{ background: C.cream, padding: "8px 20px 120px", flex: 1 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
          {[
            ...
            <div key={s.l} style={{ background: s.bg, borderRadius: 16, padding: "12px 8px", textAlign: "center", border: `3px solid ${C.navy}`, boxShadow: sh(C.navy, 2, 3) }}>
        ...
        <div style={{ background: book.disponivel ? C.green : "#DFE6E9", borderRadius: 20, padding: "16px 18px", border: `3px solid ${C.navy}`, boxShadow: sh(C.navy), display: "flex", alignItems: "center", gap: 14 }}>
...
/* ═══════════════════════════ INICIO */
// FIX: stats now loaded from real DB counts, not hardcoded zeros
function Inicio({ user, nav, loans }: { user: Profile; nav: (t: string) => void; loans: Loan[] }) {
  const [stats, setStats] = useState<UserStats>({ livros: 0, emprestados: 0, amigos: 0 });
  ...
  return (
    <main>
      ...
      <div style={{ background: C.cream, padding: "0 18px 24px" }}>
        {overdue > 0 && (
          <div role="alert" style={{ background: "#FFE5E5", borderRadius: 18, padding: "14px 18px", border: `3px solid ${C.coral}`, boxShadow: shB(), display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 22 }}>
          {[
            { n: stats.livros, l: "Livros", bg: C.yellow, i: "📚" },
            { n: stats.emprestados, l: "A Ler", bg: C.teal, i: "📖" },
            { n: stats.amigos, l: "Amigos", bg: C.lavender, i: "👫" },
          ].map(s => (
        ...
        {[
          ...
          <button key={a.tela} onClick={() => nav(a.tela)} style={{
            width: "100%", background: a.bg, borderRadius: 20, padding: "16px 18px",
...
function Biblioteca({ user }: { user: Profile }) {
  ...
  const [novo, setNovo] = useState({ titulo: "", autor: "", descricao: "", descricao_usuario: "", genero: "Outros", nota: 5, paginas: "", capa: null as string | null, prev: null as string | null });
  ...
  const load = useCallback(async () => {
    ...
    try { const b = await getMyBooks(user.id); setBooks(b); }
    // FIX: no more silent catch — show the error
    catch (e: unknown) { setErro((e as Error).message || "Erro ao carregar livros."); }
    setLoading(false);
  }, [user.id]);
  ...
  const fechar = () => {
    setModal(false);
    setNovo({ titulo: "", autor: "", descricao: "", descricao_usuario: "", genero: "Outros", nota: 5, paginas: "", capa: null, prev: null });
  ...
  const onArq = (e: React.ChangeEvent<HTMLInputElement>) => {
    const a = e.target.files?.[0];
    if (!a || !a.type.startsWith("image/")) return;
    const r = new FileReader();
    r.onload = ev => { const s = ev.target?.result as string; setNovo(p => ({ ...p, capa: s, prev: s })); };
    r.readAsDataURL(a);
  };
  ...
  if (detail) return <BookDetail book={detail} user={user} lenderId={detail.user_id} onBack={() => setDetail(null)} onRequestDone={() => setToast("📬 Pedido enviado!")} />;
  ...
  return (
    <main>
      ...
      <div style={{ background: C.cream, padding: "8px 18px 24px" }}>
        {erro && <ErrBox msg={erro} onRetry={load} />}
        {books.length > 0 && (
          <div style={{ display: "flex", gap: 8, overflowX: "auto", marginBottom: 14, paddingBottom: 4 }}>
            {generos.map(g => <button key={g} onClick={() => setFiltro(g)} aria-pressed={filtro === g} style={{ background: filtro === g ? C.navy : C.white, color: filtro === g ? C.white : C.navy, border: `2.5px solid ${C.navy}`, borderRadius: 20, padding: "7px 16px", fontFamily: "'Fredoka One',cursive", fontSize: 12, cursor: "pointer", whiteSpace: "nowrap", minHeight: 44 }}>{g}</button>)}
          </div>
        )}
        <button ref={btnRef} onClick={() => setModal(true)} style={{ width: "100%", background: C.coral, color: C.white, border: `3px solid ${C.navy}`, borderRadius: 20, padding: "14px", fontFamily: "'Fredoka One',cursive", fontSize: 18, cursor: "pointer", marginBottom: 16, boxShadow: shB(), display: "flex", alignItems: "center", justifyContent: "center", gap: 8, minHeight: 54 }}>
          ➕ Adicionar Livro!
        </button>
        {!loading && books.length === 0 && !erro
          ...
          : <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {filtrados.map(book => (
              <button key={book.id} onClick={() => setDetail(book)} aria-label={`${book.titulo}${book.autor ? ` por ${book.autor}` : ""}`} style={{ background: C.lavender + "22", borderRadius: 20, padding: "16px 14px", border: `3px solid ${C.navy}`, boxShadow: sh(C.navy), cursor: "pointer", textAlign: "center", minHeight: 44 }}>
                <Capa book={book} size={90} />
                <p style={{ fontFamily: "'Fredoka One',cursive", fontSize: 13, color: C.navy, marginTop: 8, lineHeight: 1.2 }}>{book.titulo.length > 22 ? book.titulo.slice(0, 22) + "…" : book.titulo}</p>
                <span style={{ display: "inline-block", marginTop: 8, background: book.disponivel ? C.green : C.orange, color: C.navy, borderRadius: 10, padding: "3px 10px", fontFamily: "'Fredoka One',cursive", fontSize: 10, border: `2px solid ${C.navy}` }}>{book.disponivel ? "✓ LIVRE" : "📤 EMPRESTADO"}</span>
              </button>
            ))}
          </div>
        }
        {modal && (
          <div role="presentation" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.65)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 100 }} onClick={fechar}>
            <div role="dialog" aria-modal={true} aria-labelledby="mt" onClick={e => e.stopPropagation()} style={{ background: C.cream, borderRadius: "28px 28px 0 0", padding: "20px 20px 46px", width: "100%", maxWidth: 480, border: `3px solid ${C.navy}`, borderBottom: "none", maxHeight: "90vh", overflowY: "auto" }}>
              ...
              <Inp id="n-titulo" label="Título" value={novo.titulo} onChange={v => setNovo(p => ({ ...p, titulo: v }))} placeholder="ex: Harry Potter" required />
              <Inp id="n-autor" label="Autor" value={novo.autor} onChange={v => setNovo(p => ({ ...p, autor: v }))} placeholder="ex: J.K. Rowling" />
              <Inp id="n-paginas" label="Páginas" type="number" value={novo.paginas} onChange={v => setNovo(p => ({ ...p, paginas: v }))} placeholder="ex: 320" />
              <div style={{ marginBottom: 14 }}>
                ...
                <select id="n-genero" value={novo.genero} onChange={e => setNovo(p => ({ ...p, genero: e.target.value }))} style={{ width: "100%", border: `3px solid ${C.navy}`, borderRadius: 14, padding: "12px 14px", fontSize: 15, fontFamily: "'Boogaloo',cursive", outline: "none", boxSizing: "border-box", background: C.white, boxShadow: sh(C.navy, 2, 2), color: C.navy }}>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {[1, 2, 3, 4, 5].map(n => <button key={n} onClick={() => setNovo(p => ({ ...p, nota: n }))} style={{ flex: 1, fontSize: 24, background: "none", border: "none", cursor: "pointer", opacity: n <= novo.nota ? 1 : .3 }}>★</button>)}
                </div>
              </div>
              <div style={{ marginBottom: 14 }}>
                ...
                <textarea id="n-desc" value={novo.descricao} onChange={e => setNovo(p => ({ ...p, descricao: e.target.value }))} placeholder="Sobre o que é o livro?" rows={2} style={{ width: "100%", border: `3px solid ${C.navy}`, borderRadius: 14, padding: "12px 14px", fontSize: 14, fontFamily: "'Boogaloo',cursive", outline: "none", boxSizing: "border-box", background: C.white, boxShadow: sh(C.navy, 2, 2), color: C.navy, resize: "vertical" }} />
              </div>
              <div style={{ marginBottom: 20 }}>
                ...
                <textarea id="n-nota-u" value={novo.descricao_usuario} onChange={e => setNovo(p => ({ ...p, descricao_usuario: e.target.value }))} placeholder="O que achaste? Recomendas?" rows={2} style={{ width: "100%", border: `3px solid ${C.navy}`, borderRadius: 14, padding: "12px 14px", fontSize: 14, fontFamily: "'Boogaloo',cursive", outline: "none", boxSizing: "border-box", background: C.white, boxShadow: sh(C.navy, 2, 2), color: C.navy, resize: "vertical" }} />
              </div>
...
function Emprestados({ user }: { user: Profile }) {
  ...
  const load = useCallback(async () => {
    ...
    // FIX: no more silent catch
    } catch (e: unknown) { setErro((e as Error).message || "Erro ao carregar empréstimos."); }
    setLoading(false);
  }, [user.id]);
  ...
  const handleReturn = async (loanId: string) => {
    try { await returnLoan(loanId); setToast("📦 Devolvido!"); await load(); }
    catch (e: unknown) { setToast("Erro: " + (e as Error).message); }
  };
  // FIX: removed unused bookId parameter
  const handleApprove = async (loanId: string) => {
    try { await approveLoan(loanId); setToast("✅ Empréstimo aprovado!"); await load(); }
    catch (e: unknown) { setToast("Erro: " + (e as Error).message); }
  };
  const handleReject = async (loanId: string) => {
    try { await rejectLoan(loanId); setToast("❌ Pedido recusado"); await load(); }
    catch (e: unknown) { setToast("Erro: " + (e as Error).message); }
  };
              ...
              {lent.filter(l => l.status === "requested").length > 0 && (
                <>
                  ...
                  {lent.filter(l => l.status === "requested").map(loan => (
                    <div key={loan.id} style={{ background: C.yellow, borderRadius: 20, padding: "16px", border: `3px solid ${C.navy}`, boxShadow: sh(C.navy), marginBottom: 14 }}>
                      <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 12 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <p style={{ fontFamily: "'Boogaloo',cursive", fontSize: 13, color: C.navy, opacity: .8 }}>{loan.borrower?.avatar_emoji} {loan.borrower?.nome} quer pegar emprestado!</p>
                        </div>
                      </div>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                        {/* FIX: removed unused bookId argument */}
                        <Btn onClick={() => handleApprove(loan.id)} bg={C.green} small>✓ Aceitar</Btn>
              ...
              {borrowed.length > 0 && (
                <>
                  ...
                  {borrowed.map(loan => {
                    ...
                    return (
                      <div key={loan.id} style={{ background: C.teal + "22", borderRadius: 20, padding: "16px", border: `3px solid ${C.navy}`, boxShadow: sh(C.navy), marginBottom: 14 }}>
                        ...
                        <button onClick={() => handleReturn(loan.id)} style={{ width: "100%", marginTop: 12, background: C.white, color: C.navy, border: `3px solid ${C.navy}`, borderRadius: 14, padding: "10px", fontFamily: "'Fredoka One',cursive", fontSize: 14, cursor: "pointer", boxShadow: sh(C.navy, 2, 2) }}>📦 Marcar como Devolvido</button>
                      </div>
                    );
                  })}
                </>
              ...
              {lent.filter(l => l.status === "active").length > 0 && (
                <>
                  ...
                  {lent.filter(l => l.status === "active").map(loan => (
                    <div key={loan.id} style={{ background: C.lavender + "22", borderRadius: 20, padding: "14px 16px", border: `3px solid ${C.navy}`, boxShadow: sh(C.navy), marginBottom: 12, display: "flex", alignItems: "center", gap: 12 }}>
                      {loan.book && <Capa book={loan.book} size={60} />}
                      <div style={{ flex: 1 }}>
                        <p style={{ fontFamily: "'Fredoka One',cursive", fontSize: 15, color: C.navy }}>{loan.book?.titulo}</p>
                        <p style={{ fontFamily: "'Boogaloo',cursive", fontSize: 13, color: C.navy, opacity: .8 }}>{loan.borrower?.avatar_emoji} {loan.borrower?.nome} tem agora</p>
                      </div>
                    </div>
                  ))}
                </>
              )}
              {borrowed.length === 0 && lent.length === 0 && !erro && (
...
function Amigos({ user }: { user: Profile }) {
  ...
  const [pending, setPending] = useState<Array<{ id: string; requester_id: string; profile: Profile | null }>>([]);
  ...
  const copiarLink = async () => {
    // FIX: uses modern copyToClipboard helper (no deprecated execCommand)
    const ok = await copyToClipboard(linkConvite);
  ...
  const partilharLink = async () => {
    if (navigator.share) {
      try {
        await navigator.share({ title: "BookBuddy", text: `${user.nome} convidou-te para o BookBuddy! Troca livros com amigos 📚`, url: linkConvite });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setFriends(f); setPending(p as typeof pending);
    // FIX: no more silent catch
    } catch (e: unknown) { setErro((e as Error).message || "Erro ao carregar amigos."); }
    setLoading(false);
  }, [user.id]);
  ...
  const openFriend = async (profile: Profile) => {
    ...
    try { const b = await getFriendBooks(profile.id); setFriendBooks(b); }
    catch { setFriendBooks([]); }
  };

  const handleRespond = async (id: string, action: "accepted" | "blocked") => {
    try { await respondFriendRequest(id, action); setToast(action === "accepted" ? "👫 Amigo aceite!" : "Pedido recusado"); await load(); }
    catch (e: unknown) { setToast("Erro: " + (e as Error).message); }
  };
  ...
  const enviarConvite = async () => {
    ...
    setInvErr("");
    try {
      const { error } = await supabase.functions.invoke("send-invite", { body: { email: invEmail } });
      if (error) throw error;
    } catch (e: unknown) {
      console.warn("Edge function:", (e as Error).message);
    }
    setInvDone(true);
  };

  if (detail && selFriend) return <BookDetail book={detail} user={user} lenderId={selFriend.id} onBack={() => setDetail(null)} onRequestDone={() => setToast("📬 Pedido enviado ao " + selFriend.nome + "!")} />;
        ...
        {friendBooks.length === 0
          ...
          : <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {friendBooks.map(book => (
              <button key={book.id} onClick={() => setDetail(book)} aria-label={`${book.titulo} — ${book.disponivel ? "Disponível" : "Indisponível"}`} style={{ background: C.lavender + "22", borderRadius: 20, padding: "14px 12px", border: `3px solid ${C.navy}`, boxShadow: sh(C.navy), cursor: "pointer", textAlign: "center", minHeight: 44 }}>
                <Capa book={book} size={80} />
                <p style={{ fontFamily: "'Fredoka One',cursive", fontSize: 13, color: C.navy, marginTop: 6, lineHeight: 1.2 }}>{book.titulo.length > 22 ? book.titulo.slice(0, 22) + "…" : book.titulo}</p>
  ...
  return (
    <main>
      ...
      <div style={{ background: C.cream, padding: "8px 18px 24px" }}>
        ...
        {!loading && friends.length === 0 && pending.length === 0 && !erro
          ...
          : <>
            ...
            {friends.map(f => (
              <button key={f.id} onClick={() => openFriend(f.profile)} aria-label={`Ver biblioteca de ${f.profile.nome}`} style={{ width: "100%", background: C.teal + "22", borderRadius: 20, padding: "16px 18px", border: `3px solid ${C.navy}`, boxShadow: sh(C.navy), display: "flex", alignItems: "center", gap: 14, marginBottom: 12, cursor: "pointer", textAlign: "left", minHeight: 80 }}>
        ...
        {modal && (
          <div role="presentation" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.65)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 100 }} onClick={() => setModal(false)}>
            <div role="dialog" aria-modal={true} aria-labelledby="ma" onClick={e => e.stopPropagation()} style={{ background: C.cream, borderRadius: "28px 28px 0 0", padding: "0", width: "100%", maxWidth: 480, border: `3px solid ${C.navy}`, borderBottom: "none" }}>
                ...
                {!invDone ? (
                  <>
                    <div style={{ background: C.yellow + "44", borderRadius: 16, padding: "14px 16px", border: `3px solid ${C.yellow}`, marginBottom: 18 }}>
                      ...
                      <div style={{ background: C.white, borderRadius: 12, padding: "10px 12px", border: `2px solid ${C.navy}`, marginBottom: 10, fontFamily: "'Boogaloo',cursive", fontSize: 11, color: C.navy, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        🔗 bookbuddy-eta.vercel.app?convite={user.id.slice(0, 8)}...
                      </div>
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                        <button onClick={copiarLink} style={{ background: linkCopiado ? C.green : C.white, color: C.navy, border: `2.5px solid ${C.navy}`, borderRadius: 14, padding: "10px", fontFamily: "'Fredoka One',cursive", fontSize: 13, cursor: "pointer", boxShadow: sh(C.navy, 2, 2), minHeight: 44 }}>{linkCopiado ? "✓ Copiado!" : "📋 Copiar"}</button>
                        <button onClick={partilharLink} style={{ background: C.coral, color: C.white, border: `2.5px solid ${C.navy}`, borderRadius: 14, padding: "10px", fontFamily: "'Fredoka One',cursive", fontSize: 13, cursor: "pointer", boxShadow: sh(C.navy, 2, 2), minHeight: 44 }}>📤 Partilhar</button>
                      </div>
                    </div>
...
function Notificacoes({ user, onVoltar }: { user: Profile; onVoltar: () => void }) {
  ...
  const load = useCallback(async () => {
    setLoading(true); setErro("");
    try { const n = await getNotifications(user.id); setNotifs(n); }
    catch (e: unknown) { setErro((e as Error).message || "Erro ao carregar notificações."); }
    setLoading(false);
  }, [user.id]);
  ...
  const corPorTipo: Record<string, string> = { pedido: C.teal, devolucao: C.green, amizade: C.lavender, atraso: C.coral };
  const iconePorTipo: Record<string, string> = { pedido: "📚", devolucao: "✅", amizade: "👋", atraso: "⏰" };
          ...
          : <ul style={{ listStyle: "none", padding: 0 }}>
            {notifs.map(n => (
              <li key={n.id} style={{ marginBottom: 10 }}>
                <div style={{ background: n.lida ? C.white : corPorTipo[n.tipo] + "28", border: `3px solid ${n.lida ? "#E0E0E0" : corPorTipo[n.tipo]}`, borderRadius: 18, padding: "14px", boxShadow: n.lida ? "none" : sh(corPorTipo[n.tipo], 3, 3), opacity: n.lida ? .75 : 1 }}>
                  <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ fontFamily: "'Boogaloo',cursive", fontSize: 11, color: C.navy, opacity: .5, marginTop: 4 }}>{new Date(n.created_at).toLocaleDateString("pt-PT")}</p>
                    </div>
                  ...
                  {!n.lida && <button onClick={() => ler(n.id)} style={{ marginTop: 10, background: C.yellow, color: C.navy, border: `2.5px solid ${C.navy}`, borderRadius: 12, padding: "8px 14px", fontFamily: "'Fredoka One',cursive", fontSize: 13, cursor: "pointer", minHeight: 44 }}>✓ Marcar como lida</button>}
                </div>
              </li>
            ))}
          </ul>
      ...
      <div style={{ background: C.cream, padding: "4px 18px 100px" }}>
        <div style={{ background: C.white, borderRadius: 20, border: `3px solid ${C.navy}`, boxShadow: sh(C.navy), overflow: "hidden", marginBottom: 14 }}>
          {["Privacidade 🔒", "Ajuda e Suporte 🆘"].map((item, i) => (
            <button key={item} style={{ width: "100%", padding: "16px 18px", borderTop: i > 0 ? "2px solid #F0F0F0" : "none", display: "flex", alignItems: "center", justifyContent: "space-between", background: "none", border: "none", cursor: "pointer", minHeight: 54, fontFamily: "'Fredoka One',cursive", fontSize: 15, color: C.navy, textAlign: "left" }}>
...
function InviteConfirm({ inviterId, user, onDone }: { inviterId: string; user: Profile; onDone: (success: boolean) => void }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [profile, existing] = await Promise.all([getProfileById(inviterId), checkFriendship(user.id, inviterId)]);
  ...
  const aceitar = async () => {
    setStatus("sending");
    try { await sendFriendRequest(user.id, inviterId); }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100dvh", background: C.cream }}>
      <div style={{ background: C.lavender, padding: "20px 20px 0", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, textAlign: "center", paddingBottom: 8 }}>
          ...
          <h1 style={{ fontFamily: "'Fredoka One',cursive", fontSize: 28, color: C.navy, marginTop: 8 }}>
            {status === "loading" ? "A carregar..." : status === "self" ? "Ups! 😅" : status === "already" ? "Já são amigos! 🎉" : status === "done" ? "Pedido enviado! 🎉" : `${inviterProfile?.nome || "Alguém"} quer ser teu Amigo Leitor!`}
          </h1>
        </div>
        <Wave color={C.cream} />
      </div>
      <div style={{ background: C.cream, padding: "24px 24px 40px", flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        {status === "loading" && <p style={{ fontFamily: "'Boogaloo',cursive", fontSize: 18, color: C.navy, opacity: .7 }}>A verificar convite...</p>}
        {status === "self" && (<><p style={{ fontFamily: "'Boogaloo',cursive", fontSize: 16, color: C.navy, textAlign: "center", lineHeight: 1.6, marginBottom: 24 }}>Este é o teu próprio link de convite! Partilha-o com os teus amigos.</p><Btn onClick={() => onDone(false)}>Voltar à app</Btn></>)}
        {status === "already" && (<><div style={{ fontSize: 80, marginBottom: 16, textAlign: "center" }}>{inviterProfile?.avatar_emoji || "👫"}</div><p style={{ fontFamily: "'Fredoka One',cursive", fontSize: 22, color: C.navy, marginBottom: 8 }}>{inviterProfile?.nome}</p><p style={{ fontFamily: "'Boogaloo',cursive", fontSize: 15, color: C.navy, opacity: .7, marginBottom: 24 }}>Já são Amigos Leitores!</p><Btn onClick={() => onDone(true)}>Ver a app 📚</Btn></>)}
        {status === "ready" && inviterProfile && (
          <>
            <div style={{ background: C.yellow, borderRadius: 24, padding: "24px", border: `3px solid ${C.navy}`, boxShadow: shB(), textAlign: "center", marginBottom: 24, width: "100%", maxWidth: 320 }}>
            ...
            <div style={{ background: C.green + "33", borderRadius: 16, padding: "14px 18px", border: `2.5px solid ${C.green}`, marginBottom: 24, display: "flex", gap: 10, width: "100%", maxWidth: 320 }}>
              <span aria-hidden="true" style={{ fontSize: 22 }}>🔒</span>
              <p style={{ fontFamily: "'Boogaloo',cursive", fontSize: 13, color: C.navy, lineHeight: 1.5 }}>Se aceitares, vocês poderão ver as bibliotecas um do outro e trocar livros!</p>
            </div>
        ...
        {status === "sending" && <p style={{ fontFamily: "'Boogaloo',cursive", fontSize: 18, color: C.navy, opacity: .7 }}>A enviar pedido...</p>}
        {status === "done" && (
          <>
            <div style={{ fontSize: 80, marginBottom: 16, textAlign: "center" }}>🎉</div>
            <p style={{ fontFamily: "'Fredoka One',cursive", fontSize: 22, color: C.navy, textAlign: "center", marginBottom: 8 }}>Pedido enviado a {inviterProfile?.nome}!</p>
            <p style={{ fontFamily: "'Boogaloo',cursive", fontSize: 15, color: C.navy, opacity: .7, textAlign: "center", marginBottom: 24, lineHeight: 1.6 }}>Quando {inviterProfile?.nome} aceitar, poderão ver as bibliotecas um do outro.</p>
...
export default function App() {
  ...
  useEffect(() => {
    const { data: { subscription } } = onAuthChange(async (session: unknown) => {
      ...
      if (s?.user) {
        ...
        if (p) {
          ...
          try {
            const [b, l] = await Promise.all([getMyBorrowedLoans(p.id), getMyLentLoans(p.id)]);
            setLoans([...b, ...l]);
          } catch { /* non-fatal */ }
  ...
  return (
    <>
      ...
      `}</style>
      <div lang="pt-PT" style={{ display: "flex", flexDirection: "column", minHeight: "100dvh", background: C.cream, maxWidth: 480, margin: "0 auto", position: "relative" }}>
        {fluxo === "app" && (
          <header style={{ padding: "12px 18px", background: C.cream, flexShrink: 0, borderBottom: `3px solid ${C.navy}`, display: "flex", alignItems: "center", gap: 10, position: "sticky", top: 0, zIndex: 10 }}>
